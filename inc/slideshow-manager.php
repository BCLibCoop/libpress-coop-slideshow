  ...
  class SlideshowManager
  {
    ...
    public function _init()
    {
      if (is_admin()) {
        ...
        add_action('wp_ajax_slideshow_add_text_slide', [&$this, 'slideshow_add_text_slide']);
        add_action('wp_ajax_precheck_slideshow_collection_name', [&$this, 'slideshow_precheck_collection_name']);
        add_action('wp_ajax_slideshow-fetch-img-meta', [&$this, 'slideshow_fetch_img_meta_callback']);
        add_action('wp_ajax_slideshow-fetch-collection', [&$this, 'slideshow_fetch_collection']);
        add_action('wp_ajax_slideshow-save-slide-collection', [&$this, 'slideshow_save_collection_handler']);
        add_action('wp_ajax_slideshow-delete-slide-collection', [&$this, 'slideshow_delete_collection_handler']);
      }
    }
    ...
    public function slideshow_manager_page()
    {
      ...
      $out[] = $this->slideshow_collection_selector();
      ...
      $fetched = $this->fetch_network_shared_media_slides();
      ...
      $fetched = $this->fetch_network_shared_media_slides('BC');
      $out = array_merge($out, $fetched);
      ...
      $fetched = $this->fetch_network_shared_media_slides('MB');
      $out = array_merge($out, $fetched);
    ...
    public function fetch_network_shared_media_slides($region = "")
    {
      ...
      if (empty($get_slides)) {
        return ['<div class="slide-no-results"><p>No slides</p></div>']; //we got nothing with the post meta
      }
    ...
    public function target_pages_selector()
    {
      ...
      $out = ['<select data-placeholder="Link to a post or page..." id="slideshow_page_selector" name="slideshow_page_selector" class="slideshow-page-selector chzn-select">'];
      $out[] = '<option value=""></option>';
      foreach ($res as $r) {
    ...
    private function text_slide_create_form()
    {
      ...
      $out[] = $this->target_pages_selector();
    ...
    public function slideshow_save_collection_handler()
    {
      ...
      if (empty($slideshow_id)) {
        $slideshow_id = $this->slideshow_create_collection($title);
      }
      ...
      $ret = $wpdb->update($table_name, ['slideshow_id' => 0], ['slideshow_id' => $slideshow_id]);
      ...
      $table_name = $wpdb->prefix . 'slideshow_slides';
      $ret = $wpdb->delete($table_name, ['slideshow_id' => 0]);
    ...
    public function slideshow_delete_collection_handler()
    {
      ...
      $ret = $wpdb->delete($table_name, ['slideshow_id' => $slideshow_id]);
      // error_log( 'remove from '.$table_name .': '. $ret .' for slideshow_id: '.  $slideshow_id );

      $table_name = $wpdb->prefix . 'slideshows';
      $ret = $wpdb->delete($table_name, ['id' => $slideshow_id]);
      error_log('remove from ' . $table_name . ': ' . $ret . ' for slideshow_id: ' .  $slideshow_id);
    ...
    public function slideshow_fetch_img_meta_callback()
    {
      ...
      $meta = $this->slideshow_fetch_img_meta($post_id);
    ...
    public function slideshow_add_text_slide()
    {
      ...
      if (empty($slideshow_id) || $slideshow_id == 'null') {
        if (!empty($slideshow_name)) {
          $slideshow_id = $this->slideshow_create_collection($slideshow_name);
        } else {
          echo '{"result":"failed"}';
        }
      }
